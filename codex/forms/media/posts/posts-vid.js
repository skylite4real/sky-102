...
function previewPost() {
    ...
    // Add touch event listeners to enable swiping on touch screens
    previewWrapper.addEventListener('touchstart', handleTouchStart);
    previewWrapper.addEventListener('touchmove', handleTouchMove);
    ...
    function handleStart(event) {
        ...
        isSwiping = true;
        startX = event.clientX || event.touches[0].clientX; // Use touches[0] for touch devices
    }

    function handleMove(event) {
        if (!isSwiping || swipeLocked) return;

        const currentX = event.clientX || event.touches[0].clientX; // Use touches[0] for touch devices
